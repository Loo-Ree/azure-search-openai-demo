...
class ADLSGen2SingleFileStrategy(ListFileStrategy):
    """
    Concrete strategy for single file that is located in a data lake storage account
    """
        ...
        async with DataLakeServiceClient(
            ...
            async for path in filesystem_client.get_paths(path=self.data_lake_path, recursive=True):
                ...
                if path.name not in self.file_name:
                    ...
                    logging.info(f"Found {path.name} as the new file to be processed")
